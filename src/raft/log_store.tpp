...
logStore<T>::~logStore(){
    stop_deque_thread();
    if (candidate_queue_thread.joinable()){ candidate_queue_thread.join();}
}
...
void logStore<T>::append_entry_(T request){
    ...
    if(state_ == CANDIDATE){
        ...
        if(candidate_deque_status==false){
            candidate_queue_thread=std::thread(&logStore::start_candidate_deque,this);
        }
        return;
    }
...
template <typename T>
inline void logStore<T>::process_dequed_entry(T &request,std::unique_lock<std::mutex> &raft_manager_lock){
    raft_manager_lock.lock();
